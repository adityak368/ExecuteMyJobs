...
import {fetchNumberOfAgents, fetchAgents} from 'api/api';
...
export default class Agents extends Component {
    ...
    render() {
        const connectedAgents = this.state.connectedAgents.length == 0 ? <NoAgentsFound message={'No Agents Found'}/> : this.state.connectedAgents.map((agent) =>
        ...
        const disconnectedAgents = this.state.disconnectedAgents.length == 0 ? <NoAgentsFound message={'No Agents Found'}/> : this.state.disconnectedAgents.map((agent) =>
                    <List.Item key={agent.name}>
                        <List.Icon name='github' size='large' verticalAlign='middle' />
                        <List.Content>
                            <List.Header><Link to={`/agents/${agent.name}`}><strong>{agent.name}</strong></Link></List.Header>
        ...
        const unauthorizedAgents = this.state.unauthorizedAgents.length == 0 ? <NoAgentsFound message={'No Agents Found'}/> : this.state.unauthorizedAgents.map((agent) =>
                    <List.Item key={agent.name}>
                        <List.Icon name='github' size='large' verticalAlign='middle' />
                        <List.Content>
                            <List.Header><Link to={`/agents/${agent.name}`}><strong>{agent.name}</strong></Link></List.Header>
